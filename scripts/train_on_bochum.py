...
X = load.compute_connectivity(ts)
...
p_grid = {'fsel__k': [20, 100, 200], 'model__alpha': [.5, 1., 2.], 'model__l1_ratio': [.1, .5, .9]}
...
clf = GridSearchCV(estimator=mymodel, param_grid=p_grid, cv=inner_cv, scoring="explained_variance", verbose=False, return_train_score=False, n_jobs=8)
...
print "** Score on train:\t" + str(explained_variance_score(y_pred=clf.best_estimator_.predict(X), y_true=y))
print "** Best Non-nested Score on test:\t" + str(clf.best_score_)
...
best_params = []
nested_scores_train = np.zeros(outer_cv.get_n_splits())
nested_scores_test = np.zeros(outer_cv.get_n_splits())
nested_scores_test2 = np.zeros(outer_cv.get_n_splits())
i = 0
...
print "** Nested mean score on train: " + str(nested_scores_train.mean())
print "** Nested Score on test:\t" + str(nested_scores_test.mean())
