...
X = ts #load.compute_connectivity(ts)
...
mymodel, p_grid = models.pipe_conn_scale_fsel_model()


p_grid = {'fsel__k': [30, 35, 40], 'model__alpha': [.01, .05, .1], 'model__l1_ratio': [.1,x .8]}
...
print "** Score on train:\t" + str(mean_squared_error(y_pred=clf.best_estimator_.predict(X), y_true=y))
...
for train, test in outer_cv.split(X, y):
    ...
    #nested_scores_test2[i] = mean_squared_error(y_pred=clf.predict(X[test]), y_true=y[test])
    i = i+1
